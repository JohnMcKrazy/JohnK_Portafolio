...
const es = "es";
const en = "en";
const langItems = selectorAll("[data-change]");
...
const changeLang = (lang) => {
    ...
    langItems.forEach((item) => (item.textContent = item.getAttribute(`data-${currentLang}`)));
};
const setLang = () => {
...
const swipingAnimation = () => {
    ...
    selectorAll(".swipe_animation_container_full").forEach((container) => {
        ...
        const illustrationContainersObserver = new IntersectionObserver(watchSwipeAnimationContainer, optionsIO_illustration);
        illustrationContainersObserver.observe(container);
    });
...
selectorAll(".lang_btn").forEach((btn) => btn.addEventListener("click", () => changeLang(currentLang)));
